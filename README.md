# NJU-FLA2020Fall-Project
南京大学计算机系2020秋季形式语言与自动机实现
...
实现的图灵机程序中的纸带数 N > 1。
使用你在任务一中实现的图灵机程序解析器对你实现的图灵机程序进行解析，得到对应的模拟器。
使用模拟器对合法的输入串进行模拟运行。对于符合图灵机语言要求的字符串，在第一条纸带上打印 true；对于不符合图灵机语言要求的字符串，在第一条纸带上打印 false。要求在图灵机停止运行时，第一条纸带上不出现 true 和 false 外的其他内容。
请将你的图灵机设计思路以及每个状态表达的意义以注释的形式在图灵机程序文件中进行说明。
4. 验收测试
...
源代码目录 turing-project/。仅包含源代码。请确保你源代码只依赖于 C++ 标准库；请不要将二进制文件等能够被源码生成的文件打在压缩包中。

简短的编译说明 README.md。

图灵机程序目录 programs/。目录内包含两个程序，分别命名为 case1.tm、case2.tm。

实验报告 学号-姓名.pdf。报告内容包括：分析与设计思路、实验完成度、实验中遇到的问题及解决方案、总结感想，对课程和实验的意见与建议，以及其他任何想写的内容。
